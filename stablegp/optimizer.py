...
class LBFGS(Optimizer):
    ...
    def step(self, closure):
        ...
        if self.ignore_previous_state:
            state = dict()
        else:
            print("LBFGS warning: re-using previous optimizer state. This may lead to instability.")
        ...
        # optimal condition
        if opt_cond:
            print("LBFGS termination: gradient tol satisfied prior to optimisation")
            return orig_loss
        ...
        for n_iter in itr:
            ...
            if state["n_iter"] == 1:
                ...
                ys = y.dot(s)  # y*s
                if ys == 0 or ys > 1e7:
                    print(f"LBFGS warning: rho inv = {ys.detach().item():.4E}")
                if ys > 1e-10:
                    # updating memory
            ...
            if line_search_fn is not None:
                ...
                if line_search_fn != "strong_wolfe":
                    ...
                    def obj_func(x, t, d):
                        try:
                            loss, flat_grad = self._directional_evaluate(
                                closure, x, t, d
                            )
                            if np.isnan(loss) or not np.isfinite(loss):
                                raise RuntimeError(f"NaN loss during line search with step={t:.4E}")
                            return loss, flat_grad
                        except Exception as e:
                            # directional evaluate temporarily changes param value hence we reset params if obj func fails
                            self._set_param(x)
                            raise e
                ...
                if (
                    ...
                    loss = prev_loss
                    state["n_iter"] = 0
                    # Update num evals since we skip rest of the loop
            ...
            if current_evals >= max_eval:
                termination_msg = "max func evals"
                break

            # optimal condition
            if opt_cond:
                termination_msg = "grad tol"
                break

            # lack of progress
            if d.mul(t).abs().max() <= tolerance_change:
                termination_msg = "step size tol"
                break

            if abs(loss - prev_loss) < tolerance_change:
                termination_msg = "obj fun tol"
                break
        ...
        print(f"LBFGS termination: {termination_msg}")
